...
for i, src_name in enumerate(reg_labels_gozzi):
    ...
    for j, tgt_name in enumerate(reg_labels_gozzi):
        ...
        matrix[len(reg_labels_gozzi) + i, j] = np.sum(conn[r_filt][:,np.isin(labels, l_tgt_names)]* r_weights) / np.sum(r_weights)
        matrix[i, len(reg_labels_gozzi) + j] = np.sum(conn[l_filt][:, np.isin(labels, r_tgt_names)]* l_weights) / np.sum(l_weights)
        matrix[len(reg_labels_gozzi) +i, len(reg_labels_gozzi) + j] = np.sum(conn[r_filt][:, np.isin(labels, r_tgt_names)]* r_weights) / np.sum(r_weights)
