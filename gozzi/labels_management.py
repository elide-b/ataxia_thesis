    ...
    for i in range(len(conn_oh.region_labels)):
        ...
        for j in range(len(conn_oh.region_labels)):
            ...
            if j in inds_to_merge and i in inds_to_merge:
                #print('ENTRATO 1')
                df.at[key_i_k,key_j_k] = np.mean(conn_oh.weights[np.ix_(dict_areas_inds[key_i], dict_areas_inds[key_j])])
                df.at[key_j_k, key_i_k] = np.mean(conn_oh.weights[np.ix_(dict_areas_inds[key_j], dict_areas_inds[key_i])])
                
            if i in inds_to_merge and j not in inds_to_merge:
                #print('ENTRATO 2')
                df.at[key_i_k,conn_oh.region_labels[j]] = np.mean(conn_oh.weights[dict_areas_inds[key_i], j])
                df.at[conn_oh.region_labels[j], key_i_k] = np.mean(conn_oh.weights[j, dict_areas_inds[key_i]])
            
            if j in inds_to_merge and i not in inds_to_merge:
                #print('ENTRATO 3')
                #print('Key i', key_i)
                #print('Conn oh: region labels[j]', conn_oh.region_labels[j])
                df.at[conn_oh.region_labels[i],key_j_k] = np.mean(conn_oh.weights[i, dict_areas_inds[key_j]])
                df.at[key_j_k, conn_oh.region_labels[i]] = np.mean(conn_oh.weights[dict_areas_inds[key_j], i])
