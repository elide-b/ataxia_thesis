...
for id in cc_ids:
    ...
    debug_labels[cn_ids, id] = [f"({cn}, {id} = {m_brain.weights[cn, id]}): {brain.region_labels[cn]} ← {brain.region_labels[id]}" for cn in cn_ids]

tc_ids = cc_ids + cn_ids

heatmap_mask = plot_matrix(debug_weights, zmin=0.00135, zmax=0.002, show=False, labels=debug_labels).data[0]
