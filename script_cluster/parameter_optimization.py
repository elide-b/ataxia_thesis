...
data_bold = np.empty((nJi, na, nwp, nwi, 4, nreg, N_tavg), dtype=float)
    ...
    for iJi, Ji in enumerate(Ji_range):
        for iwp, wp in enumerate(wp_range):
            for iwi, wi in enumerate(wi_range):
                ...
                (t1, _tavg), (t2, _bold) = simulate(a=a, J_i=Ji, w_p=wp, W_i=wi)
                data_tavg[ia, iJi, iwp, iwi] = np.swapaxes(_tavg, 0, 3)
                data_bold[ia, iJi, iwp, iwi] = np.swapaxes(_bold, 0, 3)

np.savez("opt_parameters_healthy.npz", tavg=data_tavg, bold=data_bold, time=t, a=a_range, J_i=Ji_range, w_p=wp_range, W_i=wi_range)
